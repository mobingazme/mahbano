...
function Cart({ onClose }) {
  ...
  return (
    <div className={`fixed top-0 left-0 w-[400px] h-full bg-white  z-50 flex flex-col  items-center animate__animated ${isOpen ? 'animate__fadeInLeft' : 'animate__fadeInRight'}`}>
      ...
      <h4 className='text-gray-900 font-bold text-2xl items-start mt-20'> لیست علاقه مندی ها</h4>
      ...
      <div className='w-full px-5 py-10'>
        <Link href='/'>
          <button className={`${currentPage === '/' ? 'hover:text-Turquoise bg-Turquoise border-Turquoise border' : ' border hover:text-B bg-B border-B'} text-white  hover:bg-white   transition-all delay-100 font-bold w-full h-9  rounded-sm my-5`}>نمایش  مورد علاقه ها</button>
        </Link>
      </div>
    </div>
  )
